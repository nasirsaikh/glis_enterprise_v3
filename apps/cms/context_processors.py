...
def site_context(request):
    try:
        ...
        for item in navigation.filter(location="header").order_by("order", "label_en"):
            ...
            if item.required_permission and not (request.user.is_authenticated and (request.user.is_superuser or request.user.has_perm(item.required_permission))):
                continue
            public_header_navigation.append({
                "label": item.localized("label"),
                "url": item.resolved_url,
                "new_window": item.opens_new_window,
            })
        portal_items = list(
            navigation.filter(location="portal")
    ...
    return {
        "glis_site": site_settings,
        "glis_theme": theme_settings,
        "glis_navigation": navigation,
        "public_header_navigation": public_header_navigation,
        "portal_navigation_sections": sections,
    }
